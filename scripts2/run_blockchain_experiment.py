...
# valores obtidos do Remix IDE (Endereço do contrato implantado)
...
contract_abi = [
    ...
    {
        "inputs": [{"internalType": "bytes", "name": "novoBiocode", "type": "bytes"}],
        "name": "guardarBiocode",
        "outputs": [],
...
# Contador para saber qual índice ler
...
# Caminho genérico para o documento
caminho_datasets = r'dataset/FVC2004/DB4_B'
seed_usuario = 123
            ...
            if arquivo.endswith(".tif"):
                ...
                if vetor is not None:
                    ...
                    if biocode is not None:
                        ...
                        res_ins = medir_desempenho(inserir_blockchain, caminho_completo, biocode)
                        writer.writerow([caminho_completo, 'insercao', 'Blockchain', res_ins['tempo_s'], res_ins['cpu_percent'], res_ins['mem_percent']])
                        
                        # Mede leitura da Blockchain
                        res_read = medir_desempenho(ler_blockchain, caminho_completo, biocode)
                        writer.writerow([caminho_completo, 'leitura_1_1', 'Blockchain', res_read['tempo_s'], res_read['cpu_percent'], res_read['mem_percent']])
                        
                        contador += 1

print(f"\nTestes da Blockchain concluídos! Resultados salvos em '{nome_arquivo_csv}'.")
